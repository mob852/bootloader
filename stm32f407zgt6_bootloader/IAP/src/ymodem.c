...
//					FlashErase(FlashDestination,FlashDestination+size); 
						EraseSomeSectors(FlashDestination,(u32)FlashDestination+size);
                    Send_Byte(ACK);
                    Send_Byte(CRC16);
                  }
                  /* Filename packet is empty, end session */
...
//						STMFLASH_Write(FlashDestination,buf_ptr,packet_length);
						STMFLASH_Write(FlashDestination, (u32 *)buf_ptr, packet_length / 4);
