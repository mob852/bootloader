...
uint8_t EraseSomePages(__IO uint32_t size, uint8_t outPutCont)
{
	...
	{
		FLASHStatus = FLASH_EraseSector(ApplicationAddress + (PAGE_SIZE * EraseCounter),VoltageRange_3);
		if(outPutCont == 1)
		{
...
/* 擦除指定地址范围的 Flash 扇区 */
uint8_t EraseSomeSectors(u32 StartAddr, u32 EndAddr)
{
    FLASH_Status status = FLASH_COMPLETE;
    u32 addrx = StartAddr; // 当前地址
    printf("Erase start: 0x%08X to 0x%08X\n", StartAddr, EndAddr);
	
		FLASH_Unlock();									//解锁 
    if (addrx < 0x1FFF0000) // 仅擦除主存储区
    {
        while (addrx < EndAddr)
        {
            // 检查当前地址是否需要擦除
            if (STMFLASH_ReadWord(addrx) != 0xFFFFFFFF) // 非空，需要擦除
            {
                u32 sector = STMFLASH_GetFlashSector(addrx); // 获取当前地址所在的扇区
                printf("Erasing sector: 0x%08x\n", addrx);

                status = FLASH_EraseSector(sector, VoltageRange_3);
                if (status != FLASH_COMPLETE) // 擦除失败
                {
                    printf("Erase error at sector %d, status: %d\n", sector, status);
                    return 0; // 返回错误
                }
								addrx = STMFLASH_GetNextSectorStart(addrx); // 跳到下一个扇区起始地址
            }else{
								addrx+=4;
